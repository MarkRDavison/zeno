...
namespace ze {
    ...
    namespace Test {
        ...
        TEST_CASE("A valid piece of text can be read", "[Xml]") {
        ...
        TEST_CASE("A simple non self closing doc validates", "[Xml]") {
            const std::string text =
                "<root><child><grandchild></grandchild></child></root>";
        ...
        TEST_CASE("A simple invalid doc validates", "[Xml]") {
            const std::string text =
                "<root>>";
        ...
        TEST_CASE("getNextTagStartIndex where there is no start returns invalid", "[Xml]") {
            REQUIRE(INVALID_INDEX == XmlDocument::getNextTagStartIndex("<root>", 1));
        }

        TEST_CASE("getNextTagStartIndex where it starts with the start returns 0", "[Xml]") {
            REQUIRE(0 == XmlDocument::getNextTagStartIndex("<root>", 0));
        }

        TEST_CASE("getNextTagStartIndex where the start is later returns correctly", "[Xml]") {
            REQUIRE(6 == XmlDocument::getNextTagStartIndex("<root></root>", 1));
        }

        TEST_CASE("Parsing single open and close element with attributes streams events as expected", "[Xml]") {
        ...
        TEST_CASE("Single layer of nesting works", "[Xml]") {
        ...
        TEST_CASE("Single layer of nesting works when document has whitespace", "[Xml]") {
...
<root attribute1='value1' attribute2="value2">
    <child c_attribute1='value1' c_attribute2="value2">
        ...
        TEST_CASE("Single layer of nesting works when document has whitespace and grandchild is self closing", "[Xml]") {
            const std::string text = R"xml(
<root attribute1='value1' attribute2="value2">
    <child c_attribute1='value1' c_attribute2="value2">
        <grandchild g_attribute1='value1' g_attribute2="value2"/>
        ...
        TEST_CASE("xml with content is streamed correctly", "[Xml]") {
            const std::string text = R"xml(
<root attribute1='value1' attribute2="value2">
    <child c_attribute1='value1' c_attribute2="value2">
        ...
        TEST_CASE("Can use default methods to stream document to tree", "[Xml]") {
            const std::string text = R"xml(
<root attribute1='value1' attribute2="value2">
    <child c_attribute1='value1' c_attribute2="value2">
        ...
        TEST_CASE("Select on basic single width tree at various depths works", "[Xml]") {
            const std::string text = R"xml(
<root>
    <child>
        <grandchild>
        ...
        TEST_CASE("SelectMany on basic multi width tree at various depths works", "[Xml]") {
            const std::string text = R"xml(
<root>
    <child>
        <grandchild>
